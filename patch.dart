...
void main() {
  final path = r'd:\Development Projects\GITHUB\printHelper\lib\tablet_view\lib\tab_projects\widgets\tablet_task_preview_popup.dart';
...
import '../../../../admin/projects/widgets/task_label_editor.dart';""");
  ...
  final oldState = """class _TabletTaskDialogState extends State<_TabletTaskDialog> {
  ...
  final newState = """class _TabletTaskDialogState extends State<_TabletTaskDialog> {
  final TextEditingController _commentController = TextEditingController();
  ...
  text = text.replaceFirst(oldState, newState);

  // 3. Header _buildHeader
  text = text.replaceFirst("""          Row(
            children: [
              Text(
                _selectedStatus,
  ...
  text = text.replaceFirst("""          const SizedBox(width: 14),
  ...
  // 4. _buildLeftPane
  text = text.replaceFirst("""  Widget _buildLeftPane() {
  ...
  text = text.replaceFirst("""_MembersRow(members: widget.task.members)""", """_MembersRow(members: _task.members)""");

  text = text.replaceFirst("""                    Container(
                      height: 38,
  ...
  text = text.replaceFirst("""                          const Icon(Icons.keyboard_arrow_down, size: 18, color: Color(0xFF6C778A)),
                        ],
                      ),
                    ),
  ...
  text = text.replaceFirst("""            child: Text(
  ...
  text = text.replaceFirst("""                  children: widget.task.labels
  ...
  text = text.replaceFirst("_AttachmentList(attachments: widget.task.attachments)", "_AttachmentList(attachments: _task.attachments)");

  // 5. Right pane saves
  text = text.replaceFirst("""          Align(
  ...
  text = text.replaceFirst("""widget.task.activities.isEmpty""", """_task.activities.isEmpty""");
  text = text.replaceFirst("""widget.task.activities.length""", """_task.activities.length""");
  text = text.replaceFirst("""widget.task.activities[index]""", """_task.activities[index]""");

  // 6. Parse Due Date function overwrite
  text = text.replaceFirst("""  String _formatDueDate(String value) {
  ...
  text = text.replaceFirst("""class _LabelChip extends StatelessWidget {
  ...
  text = text.replaceFirst("""          const Icon(Icons.close, size: 12, color: Colors.white),""", """          GestureDetector(onTap: onRemove, child: const Icon(Icons.close, size: 12, color: Colors.white)),""");
