...
class ReverbSocketService {
  ...
  void connectConversationChannel({
    ...
    debugPrint(
      "🚀 Attempting to connect Conversation Channel for conversation: $conversationId",
    );
    if (_isConnected) {
      debugPrint("⚠️ Already connected, skipping");
      return;
    }
    _isConnected = true;
    ...
    _client.onConnectionEstablished.listen((_) {
      ...
      _messageSub = _conversationChannel!.bind("message.sent").listen((event) {
        debugPrint("🔊 [Conversation Channel] message.sent event received");
        debugPrint("📦 Event data: ${event.data}");
        final data = _safeJsonDecode(event.data);
        if (data != null) {
          debugPrint("✅ Decoded message data: $data");
          onMessageReceived(data);
        } else {
          debugPrint("❌ Failed to decode message data");
        }
      });
      // 2. Message Deleted
